...
class InteractiveRouteDominanceViewer:
    ...
    def update_display(self):
        ...
        if not frame_metric.empty:
            ...
            running_avg = self.frame_metrics.iloc[:self.current_frame_idx+1]["dominance_score"].mean()
        else:
        ...
        self.ax_score.plot(frame_metrics_plot["continuous_frame"], frame_metrics_plot["dominance_score"],
                         'b-', linewidth=2.5, marker='o', markersize=4, label='Frame Dominance')
        self.ax_score.axvline(x=continuous_frame_num, color='r', linestyle='--', linewidth=3)
        self.ax_score.scatter([continuous_frame_num], [current_dominance], 
                            c='red', s=200, marker='*', zorder=5, label='Current')
        self.ax_score.set_xlabel('Frame Number', fontsize=11)
        self.ax_score.set_ylabel('Dominance Score', fontsize=11)
        self.ax_score.set_title('Frame-by-Frame Dominance', fontsize=12, fontweight='bold')
        ...
        self.ax_running.set_title(f'Running Average: {running_avg:.3f}', fontsize=12, fontweight='bold')
        self.ax_running.set_ylim(0, 1)
        ...
        if not frame_metric.empty:
            ...
            self.ax_info.text(0.5, 0.98, dom_box_text, transform=self.ax_info.transAxes,
                            fontsize=18, fontweight='bold', ha='center',
                            verticalalignment='top', family='monospace',
                            bbox=dict(boxstyle='round,pad=1.2', facecolor=dom_box_color, 
