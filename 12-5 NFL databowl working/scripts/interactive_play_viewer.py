...
class InteractivePlayViewer:
    ...
    def update_visualization(self):
        ...
        # - Covariance: [[6, 0], [0, 6]] = 6 yard radius influence zone
        # - Represents receiver's "space" or area of control (forward-looking)
        
        # Calculate direction vector from receiver to ball landing
        ball_land_x = current_frame_row.get('ball_land_x_std', current_frame_row.get('ball_land_x', receiver_x + 10))
        ball_land_y = current_frame_row.get('ball_land_y_std', current_frame_row.get('ball_land_y', receiver_y))
        ...
        # Step 2: Defender Pressure PDF - account for ALL defenders within 6 yards
        ...
        if len(all_players_frame) > 0:
            # Get all defenders
            defenders = all_players_frame[all_players_frame.get('player_side', '') == 'Defense'].copy()
            
            if len(defenders) > 0:
                # Loop through ALL defenders and sum their pressure PDFs
                for idx, defender in defenders.iterrows():
                    def_x = defender.get('x_std', defender.get('x', 0))
                    def_y = defender.get('y_std', defender.get('y', 0))
                    
                    # Skip if coordinates are invalid
                    if pd.isna(def_x) or pd.isna(def_y) or def_x == 0 or def_y == 0:
                        continue
                    
                    # Calculate separation distance from receiver
                    ...
                    if separation <= 6.0:
                        ...
                        def_pdf = multivariate_normal([def_y, def_x], [[4, 0], [0, 4]]).pdf(field_grid) * def_weight
        ...
        # Step 4: Draw Purple Contour Plot (only within 6 yards of receiver)
        ...
        contour = ax.contourf(x, y, dominance_pdf_masked, cmap='Purples', alpha=0.7, levels=15, zorder=1)
