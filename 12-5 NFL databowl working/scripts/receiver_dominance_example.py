...
def add_dominance_scores_to_dataframe(
    ...
    """
    df = training_df.copy()
    # Receiver dominance is a 0-1 metric (higher = more receiver advantage)
    df['receiver_dominance'] = np.nan
    # Also expose the same metric under a more descriptive name: receiver_pressure
    # (0–1, where 1.0 = maximum receiver pressure / advantage)
    df['receiver_pressure'] = np.nan
    ...
    for (game_id, play_id), group in grouped:
        ...
        for idx, row in group.iterrows():
            try:
                ...
                df.loc[idx, 'receiver_pressure'] = dominance
