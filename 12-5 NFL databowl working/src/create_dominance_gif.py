...
def create_dominance_gif_from_dataframe(
    ...
    for idx, (frame_idx, frame_row) in enumerate(play_data.iterrows()):
        ...
        if frame_dom is not None and not pd.isna(frame_dom):
            print(f"  Frame {idx+1}/{len(play_data)} (frame {frame_num}) - Receiver Pressure: {frame_dom:.3f}...", end='\r')
        else:
...
def create_single_frame_visualization(
    ...
    if all_players_frame is not None and len(all_players_frame) > 0:
        ...
        if len(defenders) > 0:
            # Calculate distance from receiver for each defender
    ...
    # Receiver pressure score (renamed from Dominance)
    ax.text(2, 112, f"Receiver Pressure: {dominance_percent}%",
